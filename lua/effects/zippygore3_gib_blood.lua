...
function EFFECT:Init( data )
    ...
    self.EffectKillTime = CurTime() + math.Rand(5, 7)
end
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function EFFECT:Think()
    ...
    local EnhancedSplatter = DynSplatterFullyInitialized && DynSplatterEnabledCvar:GetBool()


    if ZGM3_CVARS["zippygore3_bleed_effect"] && self.NextBloodSplat < CurTime() then

        self.NextBloodSplat = CurTime() + ( ( ZGM3_INSANE_BLOOD_EFFECTS && math.Rand(0.05, 0.1) ) or (EnhancedSplatter && math.Rand(0.2, 0.4) ) or math.Rand(0.1, 0.2) )

        local effect = ( self.Entity:GetNWString("DynamicBloodSplatter_CustomBlood_Particle", false)) or blood_particles[blood_color]
        ...
        if self.Entity:GetVelocity():LengthSqr() < 64 then
            ...
            if tr.Hit then
                ...
                if EnhancedSplatter then
                    ...
                    effectdata:SetFlags( self.Entity:GetBloodColor()+1 )
                    util.Effect("dynamic_blood_splatter_effect", effectdata, true, true )            
                elseif ZGM3_INSANE_BLOOD_EFFECTS && bc == BLOOD_COLOR_RED then
                    local effectdata = EffectData()
                    effectdata:SetEntity(self.Entity)
                ...
                elseif math.random(1, 2)==1 then
                    ...
                    materials = ( bc==BLOOD_COLOR_RED && table.Copy(blood_materials) ) or
