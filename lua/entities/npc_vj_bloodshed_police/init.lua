...
function ENT:TaserLogic()
	...
	if IsValid(en) and en:IsPlayer() then
		local can = self.HaveTaser and en:GetNW2Float("ArrestState") == 1 and (en:GetMoveType() == MOVETYPE_LADDER and math.abs(en:GetPos().z-self:GetPos().z) > 72 and math.random(1,10) == 1 or self:GetCooldown("taseruse") == 0)
		if can then
...
function ENT:VoiceLogic()
	local en = self:GetEnemy()
	if IsValid(en) and en:IsPlayer() and en:GetNW2Float("ArrestState") == 2 then
...
function ENT:ArrestLogic(ply)
    if ply:IsPlayer() and ply:GetNW2Float("ArrestState") == 1 then
        if string.find(ply:GetSVAnim(), "sequence_ron_comply_start_0") or IsValid(ply:GetRD()) then
			self.Weapon_RetreatDistance = 25
		else
			self.Weapon_RetreatDistance = self.HaveTaser and 200 or 25
		end
		if !(!self:Visible(ply) and VJ.GetNearestDistance(self, ply) > 300 or VJ.GetNearestDistance(self, ply) > 600 or string.find(ply:GetSVAnim(), "sequence_ron_comply_start_0") or IsValid(ply:GetRD())) and self.HaveTaser then
...
function ENT:OnWeaponCanFire() 
	...
	if IsValid(ply) then
		local par = ply:GetOwner()
		if ply:GetClass() == "obj_vj_bullseye" and IsValid(par) then
			ply = par
		end
...
function ENT:CustomOnMeleeAttack_AfterChecks(ent)
	if ent:IsPlayer() and ent:GetNW2Float("ArrestState") == 1 then
