...
function ENT:Think()
    local rag = self.Ragdoll
    if IsValid(rag) then
        if self.DelayRemove < CurTime() and rag:GetManipulateBoneScale(rag:LookupBone("ValveBiped.Bip01_Head1")):Length() < 1 then
            self:Remove()
        end
        local physcount = rag:GetPhysicsObjectCount()
        for i = 0, physcount - 1 do
        ...
        if IsValid(self.HeightCube) then
            ...
            local tr = util.TraceLine( {
                start = cub:GetPos(),
                endpos = cub:GetPos()-Vector(0,0,1),
                mask = MASK_SOLID,
                filter = function(ent) 
