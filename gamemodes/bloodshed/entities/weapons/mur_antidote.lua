...
SWEP.PrintName = "Universal Antidote"
SWEP.Slot = 5
...
SWEP.VElements = {
	["antidote"] = { type = "Model", model = "models/murdered/heroin/syringe_out/syringe_out.mdl", bone = "main", rel = "", pos = Vector(0, -5.41, -0.205), angle = Angle(0, -90, -30), size = Vector(1.2, 1.2, 1.2), color = Color(100, 255, 100, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:CustomPrimaryAttack()
    ...
    if SERVER then
		...
		timer.Simple(2.4, function()
			...
			MuR:GiveMessage("antidote_use", ow)
			
            MuR.SubstanceSystem:CurePoison(ow)
...
function SWEP:CustomSecondaryAttack() 
	...
	if IsValid(tar) and tar:IsPlayer() then
		...
		if SERVER then
			...
			timer.Simple(2.4, function()
				if !IsValid(self) or !IsValid(ow) then return end
				MuR:GiveMessage("antidote_use_target", ow)
				
                MuR.SubstanceSystem:CurePoison(tar)
