...
SWEP.ViewModel = "models/props_junk/garbage_plasticbottle003a.mdl"
...
function SWEP:CustomPrimaryAttack()
    ...
    if SERVER then
		...
		timer.Simple(0.5, function()
			...
			MuR:GiveMessage("ghb_use", ow)
			
            MuR.Drug:Apply(ow, "ghb")
...
function SWEP:CustomSecondaryAttack() 
	...
	if IsValid(tar) and tar:IsPlayer() then
		...
		if SERVER then
			...
			timer.Simple(0.5, function()
				if !IsValid(self) or !IsValid(ow) then return end
				MuR:GiveMessage("ghb_use_target", ow)
				
                MuR.Drug:Apply(tar, "ghb")
