...
SWEP.PrintName = "Methamphetamine"
SWEP.Slot = 5
...
SWEP.VElements = {
	["meth"] = { type = "Model", model = "models/murdered/heroin/syringe_out/syringe_out.mdl", bone = "main", rel = "", pos = Vector(0, -5.41, -0.205), angle = Angle(0, -90, -30), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:CustomSecondaryAttack() 
	...
	if IsValid(tar) and tar:IsPlayer() then
		...
		if SERVER then
			...
			timer.Simple(2.4, function()
				if !IsValid(self) or !IsValid(ow) then return end
				MuR:GiveMessage("meth_use_target", ow)
				
                MuR.Drug:Apply(tar, "meth")
